...
class FruitTreeAnalyzer:
    def __init__(self):
        ...
        # Define optimal ranges for key parameters
        self.optimal_ranges = {
            'Sugars': (2.5, 4.0),
            'Malic Acid': (0.8, 1.5),
            'Vitamin C': (0.4, 0.8),
...
"""
    ...
    def predict_tree_health(self, input_data):
        """
        ...
        try:
            ...
            required_fields = ['tree_id', 'chemical_compounds', 'environmental_factors']
            for field in required_fields:
                if field not in input_data:
                    raise ValueError(f"Missing required field: {field}")

            # Extract input data
            tree_id = input_data['tree_id']
            chemical_compounds = input_data['chemical_compounds']
            environmental_factors = input_data['environmental_factors']
            previous_dosage = input_data.get('previous_dosage')
            ...
            compound_scores = []
            for compound, value in chemical_compounds.items():
                if compound in self.optimal_ranges:
                    min_val, max_val = self.optimal_ranges[compound]
                    score = 1 if min_val <= value <= max_val else 0.5
                    compound_scores.append(score)
                    
                    results['compound_analysis'][compound] = {
                        'value': value,
                        'optimal_range': f"{min_val} - {max_val}",
                        'status': 'Optimal' if min_val <= value <= max_val else 'Sub-optimal',
                        'impact': 'Positive' if min_val <= value <= max_val else 'Negative'
                    }
            ...
            for factor, value in environmental_factors.items():
                impact = self._calculate_environmental_impact(factor, value)
                env_scores.append(impact / 100)  # Convert to 0-1 scale
                
                results['environmental_impact'][factor] = {
                    'value': value,
                    'impact_score': impact,
                    'status': 'Optimal' if impact >= 80 else 'Sub-optimal' if impact >= 60 else 'Poor'
                }

            # 3. Calculate Overall Health Score
            if compound_scores and env_scores:
                compound_avg = sum(compound_scores) / len(compound_scores)
                env_avg = sum(env_scores) / len(env_scores)
                results['health_score'] = round((compound_avg * 0.6 + env_avg * 0.4) * 100, 1)
            ...
            for compound, analysis in results['compound_analysis'].items():
                if analysis['status'] == 'Sub-optimal':
                    min_val, max_val = self.optimal_ranges[compound]
                    if analysis['value'] < min_val:
                        results['recommendations'].append({
                            'category': 'Chemical',
                            'factor': compound,
                            'issue': f'Low {compound} levels',
                            'recommendation': f'Consider increasing {compound} levels to optimal range ({min_val}-{max_val})'
                        })
                    else:
                        results['recommendations'].append({
                            'category': 'Chemical',
                            'factor': compound,
                            'issue': f'High {compound} levels',
                            'recommendation': f'Consider reducing {compound} levels to optimal range ({min_val}-{max_val})'
                        })

            for factor, analysis in results['environmental_impact'].items():
                if analysis['status'] != 'Optimal':
                    results['recommendations'].append({
                        'category': 'Environmental',
                        'factor': factor,
                        'issue': f'Sub-optimal {factor}',
                        'recommendation': f'Review and adjust {factor} management practices'
                    })

            # 6. Previous Dosage Analysis (if provided)
            if previous_dosage is not None:
                results['dosage_analysis'] = {
                    'previous_dosage': previous_dosage,
                    'effectiveness': 'Effective' if results['health_score'] > 70 else 'Ineffective',
                    'recommendation': 'Maintain current dosage' if results['health_score'] > 70 else 'Consider adjusting dosage'
                }

            return results

        except Exception as e:
            raise ValueError(f"Error in health prediction: {str(e)}")
